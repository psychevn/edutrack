...
class StudentRegistrationPage:
    def __init__(self, page: ft.Page, db_manager: DatabaseManager):
        ...
        # Form fields
        self.student_number_field = ft.TextField(
        ...
        self.section_dropdown = ft.Dropdown(
            ...
            width=350,
            options=[
                ft.dropdown.Option("1A"),
                ft.dropdown.Option("1B"),
                ft.dropdown.Option("2A"),
                ft.dropdown.Option("2B"),
        ...
        # Security questions will be loaded when view is created
    
    def initialize_ui_components(self):
        """Initialize UI components and ensure they're ready for display"""
        try:
            # Load security questions
            self.load_security_questions()
            
            # Ensure all form fields are properly initialized
            if not hasattr(self, 'student_number_field') or self.student_number_field is None:
                raise Exception("Student number field not initialized")
            
            if not hasattr(self, 'first_name_field') or self.first_name_field is None:
                raise Exception("First name field not initialized")
            
            if not hasattr(self, 'middle_name_field') or self.middle_name_field is None:
                raise Exception("Middle name field not initialized")
                
            if not hasattr(self, 'last_name_field') or self.last_name_field is None:
                raise Exception("Last name field not initialized")
                
            if not hasattr(self, 'section_dropdown') or self.section_dropdown is None:
                raise Exception("Section dropdown not initialized")
                
            if not hasattr(self, 'username_field') or self.username_field is None:
                raise Exception("Username field not initialized")
                
            if not hasattr(self, 'password_field') or self.password_field is None:
                raise Exception("Password field not initialized")
                
            if not hasattr(self, 'confirm_password_field') or self.confirm_password_field is None:
                raise Exception("Confirm password field not initialized")
                
            if not hasattr(self, 'email_field') or self.email_field is None:
                raise Exception("Email field not initialized")
                
            if not hasattr(self, 'security_question_dropdown') or self.security_question_dropdown is None:
                raise Exception("Security question dropdown not initialized")
                
            if not hasattr(self, 'security_answer_field') or self.security_answer_field is None:
                raise Exception("Security answer field not initialized")
                
            if not hasattr(self, 'create_button') or self.create_button is None:
                raise Exception("Create button not initialized")
                
            if not hasattr(self, 'back_button') or self.back_button is None:
                raise Exception("Back button not initialized")
                
        except Exception as e:
            print(f"UI initialization error: {e}")
            # Continue anyway - the form should still work with basic functionality
    ...
    def load_security_questions(self):
        ...
        try:
            ...
            # If database fails, use fallback questions
            questions = fallback_questions
        ...
        # Set default section
        self.section_dropdown.value = "1A"
    ...
    def handle_create_account(self, e):
        ...
        # Get form values
        student_number = self.student_number_field.value.strip()
        ...
        section = self.section_dropdown.value
        ...
        # Validate form
        if not all([student_number, first_name, last_name, section, username, password, confirm_password, email, security_question, security_answer]):
        ...
        # Create account
        success = self.db_manager.create_student_account(
            student_number, full_name, section, username, password, email, security_question, security_answer, photo_path
        )
        
        if success:
            self.show_success("Student account created successfully! You can now login.")
            self.clear_form()
        else:
            self.show_error("Failed to create account. Username, email, or Student Number may already exist.")
    
    def clear_form(self):
        """Clear all form fields"""
        self.student_number_field.value = ""
        ...
        if self.security_question_dropdown.options:
            self.security_question_dropdown.value = self.security_question_dropdown.options[0].key
    ...
    def get_view(self):
        """Return the student registration view"""
        ...
        return ft.View(
            ...
            [
                ft.Stack(
                    controls=[
                        # Background matching student dashboard
                        ft.Container(
                            bgcolor="#f4f1ec",
                            expand=True
                        ),
                        ...
                        [
                            ...
                            ft.Container(
                                content=ft.Column(
                                    [
                                        ft.Icon(ft.Icons.SCHOOL, size=70, color="#D4817A"),
                                        ft.Text(
                                            "Create Student Account",
                                            size=28,
